...
def tpfp_default(det_bboxes,
    ...
    #The IOU of some slender rotating frames may drop sharply with a very small disturbance.
    iou_thr=0.5
...
def creat_annotions(label_path):
    ...
    for label_name in os.listdir(label_path):
        ...
        #Create a list with multiple categories
        for i in cls_name_list:
...
if __name__ == '__main__':
    ...
    #Store all categories of detection results
    det_list=[]
    for img_name in os.listdir(img_path):
        ...
        for path, im, im0s, vid_cap, s in dataset:
            ...
            for cls_i in range(len(cls_name_list)):     
                #Traverse the prediction results and save the output results of different categories separately. If the prediction is empty, create an empty array with the same dimension (0,6). The dimension needs to be consistent when evaluating vstack later.
                for pred_i in range(len(pred)):
                    if int(pred[pred_i][6])==cls_i:
                        #Due to the image scaling, gt is the original image size, and det is the result after scaling to 640.
